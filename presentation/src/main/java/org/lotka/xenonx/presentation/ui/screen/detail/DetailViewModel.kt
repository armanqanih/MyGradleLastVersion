...
class DetailViewModel @Inject constructor(
        ...
        when (event) {
            ...
            is DetailEvent.UpdateAndDeleterCoin -> {
                viewModelScope.launch {
                    ...
                    if (coinId == null) {
                        updateCoinsDetailUseCase.invoke(event.coin)
                        _state.update {
                            it.copy(
                                snackBarVisible = true,
                                snackBarMessage = "Coin Saved Successfully"
                        ...
                        _state.update {
                            it.copy(
                                snackBarVisible = true,
                                snackBarMessage = "Coin Deleted Successfully"
                            )
                        }
                    }
                }
    ...
    private fun updateCoinDetail(coinDetail: CoinDetailModel) {
        viewModelScope.launch {
            updateCoinsDetailUseCase.invoke(coinDetail)
        }
    }

    private fun getCoinLocalById(coinId: String) {
        viewModelScope.launch {
